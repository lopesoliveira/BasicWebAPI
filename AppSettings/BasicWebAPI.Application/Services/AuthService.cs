...
namespace AppSettings.BasicWebAPI.Application.Services {
    public class AuthService : IAuthService {
        ...
        private string GenerateToken(List<Claim> claims) {
            var authSignKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_configuration["JWTKey:Secret"]!));
            //var _TokenExpiryTimeInHour = Convert.ToInt64(_configuration["JWTKey:TokenExpiryTimeInHour"]);
            var tokenDescriptor = new SecurityTokenDescriptor {
                Issuer = _configuration["JWTKey:ValidIssuer"] ,
                Audience = _configuration["JWTKey:ValidAudience"] ,
                Expires = DateTime.UtcNow.AddMinutes(50) , // _TokenExpiryTimeInHour
